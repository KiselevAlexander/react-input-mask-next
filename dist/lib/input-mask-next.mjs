...
import { useRef, useEffect, useCallback, useLayoutEffect, useImperativeHandle } from 'react';
...
function validateMaxLength(props) {
    warning(!props.maxLength || !props.mask, "react-input-mask: maxLength property shouldn't be passed to the masked input. It breaks masking and unnecessary because length is limited by the mask length.");
}
function validateMaskPlaceholder(props) {
    ...
    invariant(!mask ||
        ...
        maskPlaceholder.length === mask.length, "react-input-mask: maskPlaceholder should either be a single character or have the same length as the mask:\n" +
        "mask: ".concat(mask, "\n") +
...
function validateChildren(props, inputElement) {
    ...
    invariant(!conflictProps.length, "react-input-mask: the following props should be passed to the InputMask component, not to children: ".concat(conflictProps.join(",")));
}
...
function InputMask(props) {
    ...
    return jsx("input", __assign({ ref: inputElementRef, role: props.role || 'textbox' }, inputProps));
}
